...
describe("Get Dragged Cards", () => {
  ...
  it("should return last card when only last card is dragged", () => {
    const expectedCard = getDraggedCards(decks, lastCardToDrag);
    expect([lastCardToDrag]).toEqual(expectedCard);
  });
  it("should return whole column when first card is dragged", () => {
      const wholeColumn = decks[0];
    const expectedCards = getDraggedCards(decks, firstCardToDrag);
    expect(wholeColumn).toEqual(expectedCards);
  });
});
